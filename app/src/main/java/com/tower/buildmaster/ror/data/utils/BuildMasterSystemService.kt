...
class BuildMasterSystemService(private val context: Context) {
    ...
    suspend fun volcanoGetGaid() : String  = withContext(Dispatchers.IO){
        ...
        Log.d(BuildMasterApp.Companion.BM_MAIN_TAG, "Gaid: $gaid")
        return@withContext gaid
    }

    fun volcanoGetAppsflyerId(): String {
        val appsflyrid = AppsFlyerLib.getInstance().getAppsFlyerUID(context) ?: ""
        Log.d(BuildMasterApp.Companion.BM_MAIN_TAG, "AppsFlyer: AppsFlyer Id = $appsflyrid")
        return appsflyrid
    }
